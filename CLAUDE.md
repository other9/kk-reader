...
最終更新: 2026-08-12（お気に入り記事を30日pruneの対象から除外 update-021）
...
**お気に入りは保持期間の例外**（update-021, 2026-08-12）:
prune 前に kk-sync `/state` からお気に入り ID を取得し、state=1 の記事は
30日を過ぎても articles.json に残す。`/state` が取得できないラン
（WORKER_TOKEN 未設定・通信失敗）は prune 自体をスキップする
（一度 prune された記事は次ランで戻せないため、削らない側に倒す設計）。
ローカルで env なしに実行すると prune はスキップされるが、正の実行環境は
Actions（env 設定済み）なので問題ない。
